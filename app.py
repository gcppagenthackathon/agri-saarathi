...
def create_session():
    ...
    if not user_id or not app_name:
        return jsonify({"error": "Missing 'user_id' or 'app_name' in request"}), 400

    # Get environment variables
    PROJECT_ID = os.environ.get("GOOGLE_CLOUD_PROJECT_ID")
    LOCATION = os.environ.get("GOOGLE_CLOUD_LOCATION")

    # Use the provided app_name or fall back to environment variable
    REASONING_ENGINE_APP_NAME = os.environ.get("REASONING_ENGINE_APP_NAME")

    if not REASONING_ENGINE_APP_NAME:
        return jsonify(
            {"error": "No app_name provided and REASONING_ENGINE_APP_NAME environment variable not set"}), 400

    memory_bank_service = VertexAiMemoryBankService(agent_engine_id=REASONING_ENGINE_APP_NAME)
    try:
        ...
        if created_session and created_session.get("id"):
            ...
            return jsonify({"error": "Failed to create Vertex AI Session: No session ID returned"}), 500

    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"Failed to create Vertex AI Session: {str(e)}"}), 500
